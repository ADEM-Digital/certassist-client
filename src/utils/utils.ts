...
export function availableQuestionOnSelectedTopicsSubtopic(availableQuestions: QuestionDataType[], selectedSubTopics: (string| 0)[]) {
  ...
  if (selectedSubTopics.findIndex((subtopic) => subtopic === 0) > -1) return availableQuestions.length;

  return availableQuestions.filter((question) =>  selectedSubTopics.findIndex((subtopic) => subtopic ===question.subtopic) > -1).length
}
