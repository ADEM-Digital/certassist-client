...
export const useTestWizard = () => {
  ...
  const getQuestions = async (testValues: TestWizardValuesProps) => {
    try {
      let response = await axios.get(`${import.meta.env.VITE_API_URL}/questions/ids`, {
        params: {
          ...testValues,
        },
      });
  ...
  const submitTests = async (testValues: TestWizardValuesProps, navigate: NavigateFunction ) => {
    try {
      ...
      let response = await axios.post(`${import.meta.env.VITE_API_URL}/tests`, {
        ...
        questions: questionIds.map((questionId: string) => ({id: questionId, answer: null, correct: null})),
  ...
  return { wizardSteps, setWizardSteps, initialValues, getQuestions, submitTests };
};
